...
@section('content')
    <div class="site-content">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div class="group relative bg-white p-5 rounded-2xl border-2 border-transparent hover:border-red-400 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-sm font-bold text-gray-800 line-clamp-1 mb-2 group-hover:text-red-600 transition-colors">{{ $product->name }}</h3>
                        <div class="flex justify-between items-baseline">
                            <p class="text-red-600 text-xl font-black italic">₱{{ number_format($product->price * 0.8, 2) }}</p>
                        ...
                        <form action="{{ route('cart.add') }}" method="POST" class="relative z-20">
                            @csrf
        ...
        <div
            ...
            <!-- Search -->
            <!--<form method="GET" action="{{ route('products.index') }}" style="display:flex; gap:0;">
                ...
                @foreach($products as $product)
                    <div class="card group relative bg-white border border-gray-100 rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-300">
                        ...
                        <div class="p-6 relative z-10 flex flex-col h-full">
                            ...
                            <a href="{{ route('products.show', $product->id) }}" class="block">
                                <h2 class="text-sm font-black text-gray-900 group-hover:text-blue-600 transition-colors uppercase italic tracking-tighter leading-tight mb-2 truncate">{{ $product->name }}</h2>
                            </a>
                            <p class="text-[10px] text-gray-500 mb-4 line-clamp-2 leading-relaxed h-8">{{ Str::limit($product->description, 60) }}</p>
                            
                            <div class="mt-auto">
                                <div class="flex justify-between items-baseline mb-4">
                                    <p class="text-2xl font-black text-gray-900 italic tracking-tighter">₱{{ number_format($product->price, 2) }}</p>
                                ...
                                <div class="flex gap-2 relative z-20">
                                    ...
                                    <a href="{{ route('checkout.index', ['id' => $product->id, 'quantity' => 1]) }}" class="flex-1 bg-gray-900 text-white py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl active:scale-95 flex items-center justify-center text-center">
                                        Buy Now
                                    </a>
                                </div>
                            </div>
