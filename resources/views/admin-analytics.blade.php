...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div>
            ...
            <p class="text-3xl font-black text-gray-900">₱{{ number_format($totalRevenue, 2) }}</p>
        </div>
            ...
            @foreach($recentOrders as $order)
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    ...
                    <div class="text-right">
                        <p class="font-black text-blue-600">₱{{ number_format($order->total, 2) }}</p>
                        <span class="text-[10px] uppercase font-bold {{ $order->status === 'completed' ? 'text-green-600' : 'text-yellow-600' }}">{{ $order->status }}</span>
                    </div>
                </div>
            @endforeach
        </div>
